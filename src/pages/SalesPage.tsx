...
//     fetchSales();
//   }, [page, debouncedSearch, filterType, startDate, endDate]);
...
//     );
//   };
//   const fetchReport = async (type = "daily") => {
//     try {
...
//           <input
//             type="text"
//             placeholder="Search by customer, invoice, or product name..."
//             value={search}
//             onChange={(e) => {
...
//               setFilterType(""); // disable preset
//             }}
//             className="border px-3 py-2 rounded-lg dark:bg-slate-900"
//           />
...
//                 setFilterType(type);
//                 setPage(1);
//                 setStartDate(""); // clear custom
//                 setEndDate("");
//               }}
//               disabled={!!startDate && !!endDate} // disable if custom active
//               className={`px-4 py-2 rounded-lg capitalize ${
//                 reportType === type && !startDate
...
//               <h3 className="text-xl font-bold">₹{report.summary.totalGST}</h3>
//             </div>
...
//       {report && (
//         <div className="hidden">
//           <ReportPrint ref={reportRef} report={report} />
//         </div>
//       )}
//     </div>
//   );
...
import { useReactToPrint } from "react-to-print";
import useDebounce from "../hooks/useDebounce";
import { SalesHeader } from "../components/Sales/SalesHeader.js";
import { SalesSearch } from "../components/Sales/SalesSearch.js";
...
import ReportPrint from "../components/ReportPrint.js";
...
function SalesPage() {
  ...
  const { sales, loading, totalPages, deleteSale } = useSales(
    page,
    debouncedSearch,
    reportType,
    startDate,
  ...
  const {
    ...
    // Refresh sales after successful submission
  ...
  // Helper function for highlighting (can be moved to utils)
  // const highlightText = (text: string, search: string) => {
  //   if (!search || !text) return text;
  ...
  return (
    <div className="p-8 min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      />
      <SalesHeader showForm={showForm} onToggleForm={handleToggleForm} />
      {showForm && (
        <SaleForm
          editingId={editingId}
